...
Context Daemon
Specification v3.0 — Complete Edition
...
Table of Contents
1. Executive Summary — Problem, solution, key principles
2. Architecture Overview — System components and data flow
3. Capture System — Claude Code logs, file watching, multi-tool support
4. context.lock Specification — Core semantic snapshot format
5. Cold Start Solution — Retroactive import and bootstrap
6. Intent Inference — Async processing, cost optimization
7. Agent Discovery — CLAUDE.md sync, MCP Server
8. Git Integration — Commit linking, multi-developer support
9. Security & Privacy — Redaction, compliance modes
10. Scaling & Performance — Archiving, indexing, context budget
11. CLI Reference — Complete command reference
12. File Structure — Complete .context/ layout
...
1. Executive Summary
1.1 The Problem
When developers code with AI tools, the reasoning behind changes is systematically lost. The prompt, the
context, the rejected alternatives — all disappear after each session. Six months later, neither humans nor
AI can reconstruct why decisions were made.
...
Use for Context Daemon

message.content

string

THE PROMPT — primary capture target

sessionId

uuid

Group interactions in a session
...
Filter user prompts vs responses
...
// Default is 30, set high to preserve history

# IMPORTANT: Configure this BEFORE relying on historical import
...
The context.lock file is the semantic snapshot of project understanding. Designed to fit in AI context
windows (< 4000 tokens) while providing sufficient information to bootstrap agent understanding.
...
Pointers to ADRs with triggers

constraints

Suggested

Active rules and limits
...
project:
name: "zadig-ecommerce"
# From package.json/go.mod
domain: "Fashion e-commerce B2C"
# Inferred or manual
...
Each decision includes triggers — keywords that help agents identify when a decision is relevant. This
enables semantic retrieval without full-text search.
decisions:
- id: "ADR-001"
...
The cold start problem — no value on day 1 — is solved through retroactive import of existing Claude
Code history and intelligent project bootstrapping.
...
Install → empty → no value

Install → import 90d → ctx blame works

7

Some context, limited value

Rich context + new captures
...
6. Intent Inference
Inference is async and optional. The system provides value without any LLM through ctx blame and
search. Inference adds structured intent and auto-generated ADRs.
...
Returns overview, stack, constraints
...
■■■ 2026-02-04_alice_ghi789.jsonl
■■■ ...

# Alice's session
# Bob's session (same day, no conflict)
# Alice's new session

# ctx log aggregates all files at read time
$ ctx log
# Shows all
$ ctx log --author alice
# Filter by author
...
Intent timestamp vs commit timestamp

File overlap

0.5

Files in intent vs files in commit

Message similarity

0.2

Intent summary vs commit message
...
hot_storage: 90d
warm_storage: 1y
cold_storage: forever

# Individual files, fast search
# Aggregated by month, compressed
# Annual archive, external
...
Initialize .context/ with detection
...
.context/
■■■ MANIFEST.md
# Entry point (human + AI)
■■■ context.lock
# Semantic snapshot
■
■■■ intents/
# Per-session capture (one file per session)
■
■■■ 2026-02-03_alice_abc123.jsonl
■
■■■ 2026-02-03_bob_def456.jsonl
